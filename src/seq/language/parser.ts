import { any, Context, discard, eof, Error, Failure, filterNull, manyZero, many, map, must, optional, pair, regex, sequence, str, terminated } from './combi';
 ...
 */
const label = map(filterNull(sequence([
    discard(str('label:')),
    skipZero,
    must(restOfLine),
])), ([label]) => label);
